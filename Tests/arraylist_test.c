...
#include "../Data Structures/ArrayList/list.h"
#include "test.h"
...
static char *empty_array_list_test() {
  ...
  mu_assert("Did not return error message stating positive capacity needed.",
            strcmp("Capacity must be positive.",
                   initialize_array_list(&list, 0, 0)) == 0);

  mu_assert("Did not return error message for invalid width.",
            strcmp("Invalid width. Use sizeof() operator to determine width of "
                   "datatype.",
                   initialize_array_list(&list, 1, 0)) == 0);

  // Check if list initializes properly
  mu_assert("Could not initialize properly.",
...
static char *addition_to_list_works() {
  ...
  mu_assert("Failed to initialize properly",
            initialize_array_list(&list, capacity, width) == NULL);

  for (size_t index = 0; index < length; index++) {
    mu_assert("Failed to insert element in array list.",
              append_to_array_list(&list, (void *)(elements + index)) == NULL);
  }
  ...
  mu_assert("Could not add elements of array.",
            add_all_to_array_list(&list, more, more_length) == NULL);

  // Check again
  mu_assert("Invalid state after addition: length does not match.",
            list.length == (length + more_length));
  mu_assert("Invalid state after addition: capacity does not match.",
            list.capacity == 20);
 ...
 */
static char *remove_file(char *fname) {
  return remove(fname)
             ? "There was an error while trying to delete the temporary file."
             : NULL;
}

static void print_double(FILE *out_stream, const size_t index,
                         const void *element) {
...
static char *output_works_correctly() {
  ...
  mu_assert("Could not initialize properly.",
            initialize_array_list(&list, capacity, width) == NULL);
  mu_assert("Could not add all the elements to the list.",
            add_all_to_array_list(&list, values, capacity) == NULL);
  ...
  mu_assert("Could not output to temp_file.",
            output_array_list(temp_file, &list, print_double) == NULL);

  fclose(temp_file);
  // Data written to temp_file successfully. Now read to ensure that the data
  // matches.
  ...
  for (size_t index = 0; index < capacity; index++) {
    ...
    mu_assert("Content data does not match.",
              fabs(data - values[index]) <= tolerance);
  }

  fclose(temp_file);
  mu_assert("Could not remove the temporary temp_file.",
            remove_file(filename) == NULL);

  mu_assert("Could not free the ArrayList.", free_array_list(&list) == NULL);
  return NULL;
}

static void print_uint64_t(FILE *out_stream, const size_t index,
                           const void *element) {
...
static char *insertion_and_deletion_work() {
  ...
  mu_assert("Could not initialize ArrayList",
            initialize_array_list(&list, capacity, width) == NULL);

  mu_assert("Error while adding all the elements to the list.",
            add_all_to_array_list(&list, initial_contents, count) == NULL);
  mu_assert("Error while inserting element into the list.",
            insert_in_array_list(&list, 3, &element) == NULL);

  mu_assert("Could not delete data properly.",
            delete_index_array_list(&list, 4) == NULL);
  ...
  mu_assert("Failed to write the list's data to file.",
            output_array_list(temp_file, &list, print_uint64_t) == NULL);
...
static void print_uint8_t(FILE *out_stream, const size_t index,
                          const void *element) {
...
static char *set_and_get_work() {
  ...
  mu_assert("Error while adding all the elements to the list.",
            add_all_to_array_list(&list, initial_contents, count) == NULL);
  mu_assert("Error while setting an element of the list.",
            set_in_array_list(&list, set_idx, &element) == NULL);

  uint8_t result;
  mu_assert("Error while trying to obtain data from ArrayList.",
            get_from_array_list(&list, get_idx, (void *)&result) == NULL);

  mu_assert("Value obtained did not match.",
            result == initial_contents[get_idx]);
  ...
  mu_assert("Failed to write the list's data to file.",
            output_array_list(temp_file, &list, print_uint8_t) == NULL);
...
static const bool is_target(void *element) { return *((int *)element) == target; }
...
static char *search_and_comparison_work() {
  ...
  mu_assert("Search should have been unsuccessful.",
            strcmp(search_array_list(&list1, &location, is_target),
                   "Search unsuccessful") == 0);
  ...
  mu_assert("Could not deallocate arraylist 1.",
            free_array_list(&list1) == NULL);
  mu_assert("Could not deallocate arraylist 2.",
            free_array_list(&list2) == NULL);

  return NULL;
}

static char *operate_on_int(void *result, const size_t index,
                            const void *element) {
...
static char *average_calculation_works() {
  ...
  ArrayList list = {};
  mu_assert("Could not initialize array list.",
            initialize_array_list(&list, size, width) == NULL);
  mu_assert("Could not add elements to the array list.",
            add_all_to_array_list(&list, elements, size) == NULL);
